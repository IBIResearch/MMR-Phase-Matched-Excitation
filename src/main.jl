#################################################
# Phase-Matched Chirps for Energy-Efficient 
# Excitation of Magneto-Mechanical Resonators		
#################################################
# Julia: 1.11
using Pkg
Pkg.activate(".")
...
#####################################################################################################
### Run simulations for all excitation algorithms and analyze the results
#####################################################################################################


maxDeflectionAnglesDistribution = zeros(length(algs), noSamples,sequence.numFrames)
for i in 1:noSamples
	@info "Running simulation $i of $noSamples"
	# randomize the frequency and phase uncertainty 
	uncertainty_f_startRx = 2 * rand(sequence.numFrames) .- 1
	uncertainty_f_endRx = 2 * rand(sequence.numFrames) .- 1
	uncertainty_ψ_endRX = deg2rad.(10 * (2 * rand(sequence.numFrames) .- 1))
	for (algIndex, alg) in enumerate(algs) 
		# initial settings for first frame
		alg.f_TX_Array[1] = ω_nat/ (2*π) 	+ uncertainty_f_startRx[1] 								# initial frequency of excitation (in Hz)
		sequence.ψ_TX_Array[1] = 0.0 + uncertainty_ψ_endRX[1]												# initial phase of excitation (in rad)
		param = "$(shortName(alg))"
		...
		# run simulation
		deflectionAngleAllFrames, deflectionAngleAllFramesDot = simulate_meas(frameTime, sensor, sequence, uncertainty_f_startRx = uncertainty_f_startRx, uncertainty_f_endRx = uncertainty_f_endRx, uncertainty_ψ_endRX = uncertainty_ψ_endRX);
		maxDeflectionAnglesDistribution[algIndex,i,:] = rad2deg.(maximum(deflectionAngleAllFrames, dims=1))
		
		if i == noSamples
			# Calculate excitation-response phase offset at each zero crossing only for one sample
			phaseDiff = Float64[]
		...
		if i == noSamples
			...
			if isa(alg, ChirpSin_Excitation) && i == noSamples
				txTimes = range(0, sequence.t_TX, step=1/sequence.fs)
			...
			# maxDeflectionAngles = rad2deg.(maximum(deflectionAngleAllFrames, dims=1))
				...
				meanMaxDeflectionAngles = mean(maxDeflectionAnglesDistribution[algIndex,:,:], dims=1)[:]
				stdMaxDeflectionAngles = std(maxDeflectionAnglesDistribution[algIndex,:,:], dims=1)[:]
				lines!(axMaxAngle, 1:sequence.numFrames, meanMaxDeflectionAngles, label = "$(param)", color=colorAlgs[algIndex],linestyle = (algIndex <4 ? :dash : :solid))
				band!(axMaxAngle, 1:sequence.numFrames, meanMaxDeflectionAngles .- stdMaxDeflectionAngles, meanMaxDeflectionAngles .+ stdMaxDeflectionAngles, color = (colorAlgs[algIndex], 0.2))
...
#####################################################################################################
### Analyze the effect of duty cycle on the maximum deflection angle for pulsed excitation algorithms
#####################################################################################################
alg = algPulsed
